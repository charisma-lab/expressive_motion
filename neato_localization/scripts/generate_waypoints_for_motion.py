...
class BehaviorGenerator:
    ...
    def TSI_callback(self, val):
        ...
        amplitude = rospy.get_param('amplitude_slope', 0.175) * self.space_val + rospy.get_param('amplitude_intercept', 0.175)
        # when time_val = -1, then vel_factor = 0.5 and when time_val = +1 then vel_factor = 1

        velocity_factor = rospy.get_param('velocity_slope', 0.25)*self.time_val + rospy.get_param('velocity_intercept', 0.75)
        ...
        if self.time_val != 0 and self.space_val != 0 and self.interaction_val != 9: 
            ...
            noise = rospy.get_param('interaction_slope', 25) * self.interaction_val + rospy.get_param('interaction_intercept', 25)
            theta = theta + noise
            # when interaction_val = -1 then num_waypt = 25 and for +1 it is 75. This is when space_val = 1 (undulating/sine wave)
            num_waypt_count = -rospy.get_param('num_waypt_slope', 25)*self.interaction_val + rospy.get_param('num_waypt_intercept', 50)
